...
alter table public.products add constraint products_price_type_check
  check (price_type in ('fixed','negotiable','at_most'));

-- 为访客浏览器保存五分钟限流窗口；仅供安全函数使用，前台无直接权限。
...
create index if not exists reservation_rate_limits_updated_idx
  on public.reservation_rate_limits(updated_at);
...
-- 替换旧的四参数预定接口；限流在数据库内执行，不能绕过前端限制。
drop function if exists public.reserve_product(uuid,text,text,text);
...
begin
  ...
  insert into public.reservation_rate_limits(client_id) values(p_client_id)
...
group by p.id,c.id;
revoke all on public.product_feed from public;
