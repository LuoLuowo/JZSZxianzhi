...
function sanitizeIntroductionHtml(value) {const template=document.createElement('template');template.innerHTML=String(value||'');const allowed=new Set(['STRONG','B','BR','DIV','P']);const clean=node=>{if(node.nodeType===Node.TEXT_NODE)return document.createTextNode(node.textContent||'');if(node.nodeType!==Node.ELEMENT_NODE)return document.createDocumentFragment();const fragment=document.createDocumentFragment();for(const child of [...node.childNodes])fragment.append(clean(child));if(!allowed.has(node.tagName))return fragment;const tag=node.tagName==='B'?'strong':node.tagName.toLowerCase();const element=document.createElement(tag);element.append(fragment);return element;};const output=document.createElement('div');for(const child of [...template.content.childNodes])output.append(clean(child));return output.innerHTML;}
...
async function deleteProduct(id) { const product=state.products.find(p=>p.id===id); const {error}=await db.from('products').delete().eq('id',id); if(error)return toast(errorText(error),false); if(product?.image_url){const marker='/storage/v1/object/public/product-images/';const i=product.image_url.indexOf(marker);if(i>=0)await db.storage.from('product-images').remove([decodeURIComponent(product.image_url.slice(i+marker.length))]);} toast('商品已删除，对接码已永久保留'); await refreshData(); }
