...
const viewMeta = {
  overview:['数据概览','查看平台实时运营数据'],products:['商品管理','查询商品、库存、交换微信和商品码'],submissions:['发布审核','审核普通用户提交的闲置'],campusWall:['校园墙审核','审核、置顶和管理校园墙投稿'],
...
async function refreshData() {
  ...
  const results=await Promise.all([
    ...
    db.from('site_settings').select('admin_wechat,announcement,hero_headline,hero_subtitle,introduction_content,introduction_image_url').eq('id',true).maybeSingle(),
...
function renderResources() {
  ...
  state.wallPosts.filter(p=>p.image_url).forEach(p=>imageItems.push({title:`校园墙：${p.title}`,url:p.image_url,code:'校园墙'}));
...
async function reviewWallPost(id,action){const {error}=await db.rpc('admin_review_campus_wall_post',{p_post_id:id,p_action:action});if(error)return toast(errorText(error),false);toast(action==='approved'?'校园墙投稿已发布':'校园墙投稿已拒绝');await refreshData();}
async function setWallPinned(id,value){const {error}=await db.from('campus_wall_posts').update({is_pinned:value}).eq('id',id).eq('status','approved');if(error)return toast(errorText(error),false);toast(value?'校园墙帖子已置顶':'已取消置顶');await refreshData();}
async function deleteWallPost(id){const item=state.wallPosts.find(post=>post.id===id);const {error}=await db.from('campus_wall_posts').delete().eq('id',id);if(error)return toast(errorText(error),false);const path=storagePath(item?.image_url);if(path?.startsWith('wall-submissions/'))await db.storage.from('product-images').remove([path]);toast('校园墙投稿已删除');await refreshData();}
...
function bindEvents() {
  ...
  $('adminPageContent').addEventListener('click',event=>{
    ...
    if(wallReview){const item=state.wallPosts.find(post=>post.id===wallReview.dataset.reviewWall);const approved=wallReview.dataset.wallAction==='approved';return askConfirm(approved?'同意校园墙投稿':'拒绝校园墙投稿',`确定${approved?'发布':'拒绝'}“${item?.title||'该投稿'}”吗？`,approved?'同意发布':'确认拒绝',()=>reviewWallPost(wallReview.dataset.reviewWall,wallReview.dataset.wallAction),approved);}
    const wallPin=target.closest('[data-pin-wall]');
    if(wallPin){const value=wallPin.dataset.pinValue==='true';const item=state.wallPosts.find(post=>post.id===wallPin.dataset.pinWall);return askConfirm(value?'置顶校园墙帖子':'取消帖子置顶',`确定${value?'置顶':'取消置顶'}“${item?.title||'该投稿'}”吗？`,value?'确认置顶':'取消置顶',()=>setWallPinned(wallPin.dataset.pinWall,value),value);}
    const wallDelete=target.closest('[data-delete-wall]');
    if(wallDelete){const item=state.wallPosts.find(post=>post.id===wallDelete.dataset.deleteWall);return askConfirm('删除校园墙投稿',`确定永久删除“${item?.title||'该投稿'}”吗？图片也会一并清理，此操作无法恢复。`,'永久删除',()=>deleteWallPost(wallDelete.dataset.deleteWall));}
